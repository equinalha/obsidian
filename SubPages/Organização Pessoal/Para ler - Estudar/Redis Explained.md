...
---
[https://architecturenotes.co/redis/?s=09](https://architecturenotes.co/redis/?s=09)
...
5. Monitoring **— **ensuring main and secondary instances are working as expected.
6. Notification **— **notify system admins about occurrences in the Redis instances.
7. Failover management — Sentinel nodes can start a failover process if the primary instance isn't available and enough (quorum of) nodes agree that is true.
8. Configuration management — Sentinel nodes also serve as a point of discovery of the current main Redis instance.
...
9. What if the sentinel nodes fall out of quorum?
10. What if there is a network split which puts the old main instance in the minority group? What happens to those writes? (Spoiler: they are lost when the system recovers fully)
11. What happens if the network topologies of sentinel nodes and client nodes (application nodes) are misaligned? 😬
